...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        ...
        List<Question> questionLevelWise = questionMap.values().stream().filter(question -> level.equals(question.getLevel())).collect(Collectors.toList());

        if(questionLevelWise.size() == 0){
            return new ArrayList<>();
        }
